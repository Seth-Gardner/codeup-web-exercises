...
function endsWithVowel(str) {
    var chf = str.charAt(str.length - 1);
    return isVowel(chf);
}
...
function startsAndEndsWithVowel(str) {
    var ch1 = str.charAt(0);
    var ch2 = str.charAt(str.length - 1);
...
assert(median([1, 5, 6]), 4.0, "Exercise 62");
...
function highestPriceBook(x){
    var highestPriceBook = {
        'title': "Yeah im a real book",
        'author': "john doe",
        'price': '00.00'
    }
    for(var i = 0; i < x.length; i++){
        if(x[i].price > highestPriceBook.price){
            highestPriceBook = x[i]
        }
    }
...
function lowestPriceBook(x){
    var lowestPriceBook = {
        title: 'I dont read books',
        price: 999.00,
        author: "John Adams"
    }
    for(var i = 0; i < x.length; i++){
        if(x[i].price < lowestPriceBook.price){
            lowestPriceBook = x[i]
        }
    }
...
function totalNumberOfItems(x){
    ...
    for (let i = 0; i < x.items.length; i++) {
         total = total + x.items[i].quantity
    }
    return total
}
...
function getAverageItemPrice(x){
    let total = 0;
    for (let i = 0; i < x.items.length; i++) {
        total = total + x.items[i].price;
    }
    return total / x.items.length
}
...
function getAverageSpentPerItem(x) {
    ...
    for (let i = 0; i < x.items.length; i++) {
        ...
        totalAmount += x.items[i].quantity;
    }
